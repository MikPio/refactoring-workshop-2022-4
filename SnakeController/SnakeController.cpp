      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        ...
        while (length--) {
            Segment seg;
            istr >> seg.x >> seg.y;
            snake_segments.m_segments.push_back(seg);
        }
    } else {
        throw ConfigurationError();
    }
...
{
    snake_segments.updateSegmentsIfSuccessfullMove(snake_segments.calculateNewHead());
}
...
{
    if (snake_segments.isSegmentAtPosition(x, y) || isPositionOutsideMap(x,y)) {
